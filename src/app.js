...
export function namumarkToHTML(text){
  ...
  let html = DOMPurify.sanitize(text, {ALLOWED_TAGS:['h1','h2','h3','h4','h5','h6','strong','em','a','img','br','ul','li','ol','pre','code','blockquote','hr','table','tr','td','th','sup','div','section']})
...
export async function uploadFile(){
  ...
  reader.onload=e=>{
    ...
    const name=f.name
    const now=Date.now()
    const key='dambi_file_'+now+'_'+name
    localStorage.setItem(key, data)
    const url='/dambi_files/'+key
    editContent.value+='[파일:'+url+']'
...
export function openAdmin(){
  ...
  html += '<h3>등록된 사용자</h3>'
  if(users.length===0){
    html += '<p>등록된 사용자가 없습니다.</p>'
  } else {
    html += '<table><tr><th>이메일</th><th>액션</th></tr>'
    users.forEach(u=>{
      html += '<tr><td>'+escapeHtml(u.email)+'</td><td><button class="btn danger" onclick="window.adminDeleteUser(\''+encodeURIComponent(u.email)+'\')">삭제</button></td></tr>'
    })
    html += '</table>'
  }
  html += '<button class="btn" onclick="window.appLoadPage(\'환영합니다\',\'ko\')" style="margin-top:8px">돌아가기</button></div>'
